...
Status BlackWidow::CompactKey(const DataType& type, const std::string& key) {
  ...
  if (type == kSets) {
    if (key != "__shanks_epoch__"){
       LOG(WARNING) << "[CompactRange] begin CompactRange set key:" << key;
       sets_db_->CompactRange(&slice_meta_begin, &slice_meta_end, kMeta);
       sets_db_->CompactRange(&slice_data_begin, &slice_data_end, kData);
    }
    sets_db_->CompactOldFiles();
  } else if (type == kZSets) {
    if (key != "__shanks_epoch__"){
       LOG(WARNING) << "[CompactRange] begin CompactRange zset key:" << key;
       zsets_db_->CompactRange(&slice_meta_begin, &slice_meta_end, kMeta);
       zsets_db_->CompactRange(&slice_data_begin, &slice_data_end, kData);
    }
    zsets_db_->CompactOldFiles();
  } else if (type == kHashes) {
    if (key != "__shanks_epoch__"){
       LOG(WARNING) << "[CompactRange] begin CompactRange hash key:" << key;
    ...
    hashes_db_->CompactOldFiles();
  } else if (type == kLists) {
    if (key != "__shanks_epoch__"){
       LOG(WARNING) << "[CompactRange] begin CompactRange list key:" << key;
       lists_db_->CompactRange(&slice_meta_begin, &slice_meta_end, kMeta);
       lists_db_->CompactRange(&slice_data_begin, &slice_data_end, kData);
    }
    lists_db_->CompactOldFiles();
  } else if (type == kStrings) {
    if (key != "__shanks_epoch__"){
       LOG(WARNING) << "[CompactRange] begin CompactRange string key:" << key;
       strings_db_->CompactRange(&slice_meta_begin, &slice_meta_end, kMeta);
    }
    strings_db_->CompactOldFiles();
  }
  return Status::OK();
}

void BlackWidow::DealSlowKey(std::vector<std::string> argv){
...
void BlackWidow::db_compact_old_files(){
    if (bg_tasks_queue_.size() > 0){
        return;
    }
    std::string key = "__shanks_epoch__";
    LOG(WARNING) << "begin to compact strings old files : ";
    AddBGTask({DataType::kStrings, Operation::kCompactKey, key});
    AddBGTask({DataType::kHashes, Operation::kCompactKey, key});
    AddBGTask({DataType::kSets, Operation::kCompactKey, key});
    AddBGTask({DataType::kZSets, Operation::kCompactKey, key});
    AddBGTask({DataType::kLists, Operation::kCompactKey, key});
}
