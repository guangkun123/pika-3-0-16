...
#undef DeleteFile
  ...
  virtual void CompactFamiliesOldFiles(ColumnFamilyHandle* column_family1, ColumnFamilyHandle* column_family2, std::string & sst_name, int *rate){ 
      ColumnFamilyMetaData meta1, meta2;
      CompactionOptions compact_options_;
      compact_options_.output_file_size_limit = 20971520;
      GetColumnFamilyMetaData(column_family1, &meta1);
      GetColumnFamilyMetaData(column_family2, &meta2);
      ...
      *rate = 0;
      ...
      ColumnFamilyHandle * min_family = column_family1;

      for (auto level_meta : meta1.levels) {
        for (auto file_meta : level_meta.files) {
            //std::cout << "level:" << level_meta.level << ", name:" << file_meta.name << ", smallestkey:" << file_meta.smallestkey << ", largestkey:" << file_meta.largestkey << std::endl;
            db_path = file_meta.db_path;
            number = TableFileNameToNumber(file_meta.name);
            if ((number < min_number) && (level_meta.level != 0) ){
                min_family = column_family1;
      ...
      for (auto level_meta : meta2.levels) {
        for (auto file_meta : level_meta.files) {
            //std::cout << "level:" << level_meta.level << ", name:" << file_meta.name << ", smallestkey:" << file_meta.smallestkey << ", largestkey:" << file_meta.largestkey << std::endl;
            db_path = file_meta.db_path;
            number = TableFileNameToNumber(file_meta.name);
            //if ((number < min_number) && (level_meta.level != 6) ){
            if ((number < min_number) && (level_meta.level != 0) ){
                min_family = column_family2;
                min_file = file_meta.name;
                min_level = level_meta.level;
                min_number = number;
                //break;
      ...
      CompactFiles(compact_options_, min_family, input_file_names, min_level);
      *rate = sstCompactRate;
  }

  virtual void CompactOldFiles(std::string & sst_name, int *rate){ 
      ColumnFamilyMetaData meta;
      CompactionOptions compact_options_;
      compact_options_.output_file_size_limit = 20971520;
      GetColumnFamilyMetaData(DefaultColumnFamily(), &meta);
      std::vector<std::string> input_file_names;
      std::string min_file = "999999.sst";
      std::string db_path;
      *rate = 0;
      ...
      for (auto level_meta : meta.levels) {
        for (auto file_meta : level_meta.files) {
            db_path = file_meta.db_path;
            number = TableFileNameToNumber(file_meta.name);
            //std::cout << "level:" << level_meta.level << ", name:" << file_meta.name << ", number:" << number << ", min_file:" << min_file << std::endl;
            if ( ( number < min_number) && (level_meta.level != 0) ){
                min_file = file_meta.name;
                min_level = level_meta.level;
                min_number = number;
      ...
      CompactFiles(compact_options_, DefaultColumnFamily(), input_file_names, min_level);
      *rate = sstCompactRate;
  }
