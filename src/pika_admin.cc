...
void InfoCmd::InfoData(std::string &info) {
  ...
  int64_t now_t = slash::NowMicros() / 1000000;

  if (now_t - 60 > g_pika_server->db()->db_size_last_time){
      db_size = slash::Du(g_pika_conf->db_path());
      g_pika_server->db()->db_size_last_time = now_t;
      g_pika_server->db()->db_size = db_size;
      g_pika_server->db()->GetUsage(blackwidow::PROPERTY_TYPE_ROCKSDB_TABLE_READER, &table_reader_usage);
      g_pika_server->db()->table_reader_usage = table_reader_usage;
      g_pika_server->db()->db_compact_old_files();
  }else{
      db_size = g_pika_server->db()->db_size;
      table_reader_usage = g_pika_server->db()->table_reader_usage;
  }
